...
#Breakpoints
for (x in names(data)) {
  obj <-(data[[x]]$oilprice~lag(data[[x]]$oilprice))
  obj_1 <-(data[[x]]$ree~lag(data[[x]]$ree))
  cat(str_to_upper(x))
  qlr<-Fstats(obj, data=data[[x]])
  print(paste("Dla kraju ", x,"i zmiennej oil price,  structural break jest przy obserwacji: ",breakpoints(qlr)$nobs, sep=" "))
  cat("/n")
  qlr1<-Fstats(obj, data=data[[x]])
  print(paste("Dla kraju ", x,"i zmiennej ree, structural break jest przy obserwacji: ", ":", breakpoints(qlr1)$nobs, sep=" "))
  cat("/n")

}
data["fr"]
data["fr"]$fr
h<-breakpoints(data[["fr"]]$oilprice~lag(data[["fr"]]$oilprice))
h$nobs
h
j<-summary(h)
j$breakdates
