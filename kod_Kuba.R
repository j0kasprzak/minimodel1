...
for (x in kraje) {
  kraj <- kraje1[i]
  for (q in seq(0.1, 0.8, 0.1)) {
    q_reg <- rq(ree_r~oilprice_r+D08+D20, tau = c(q,q+0.1), data=data[[x]][-1,] %>% mutate(oilprice_r=oilprice_r*100, ree_r=ree_r*100))
    try({
    ...
    while(is.null(obj) & iter<200) {
      try({
        q_reg <- rq(ree_r~oilprice_r+D08+D20, tau = c(q+runif(1, -0.02, 0.02),q+0.1+runif(1, -0.04, 0.04)), data=data[[x]][-1,] %>% mutate(oilprice_r=oilprice_r*100, ree_r=ree_r*100))
        obj <-anova(q_reg, test = "Wald", joint=F)
      }, silent = T)
      iter <- iter+1
    }
    if (!is.null(obj)) {
      ...
      df_qr_test[df_qr_test$Country==kraj,10*q+2] <- NA
      obj <- NULL
    }
    iter=0
    cat('q=',q)
  }
  q_reg <- rq(ree_r~oilprice_r+D08+D20, tau = c(0.1,0.5), data=data[[x]][-1,] %>% mutate(oilprice_r=oilprice_r*100, ree_r=ree_r*100))
  obj <-anova(q_reg, test = "Wald", joint=F)
  df_qr_test[df_qr_test$Country==kraj,11] <- round(obj$table$pvalue,4)
  obj <- NULL
  
  #q_reg <- rq(ree_r~oilprice_r+D08+D20, tau = c(0.5,0.9), data=data[[x]][-1,] %>% mutate(oilprice_r=oilprice_r*100, ree_r=ree_r*100))
  #obj <-anova(q_reg, test = "Wald", joint=F)
  #df_qr_test[df_qr_test$Country==kraj,12] <- round(obj$table$pvalue,4)
  obj <- NULL
  print(i)
  i <- i+1
}
...
for (x in kraje) {
  ...
  while(is.null(obj) & iter<200) {
    try({
      q_reg <- rq(ree_r~oilprice_r+D08+D20, tau = c(0.5+runif(1, -0.02, 0.02),0.9+runif(1, -0.04, 0.04)), data=data[[x]][-1,] %>% mutate(oilprice_r=oilprice_r*100, ree_r=ree_r*100))
      obj <-anova(q_reg, test = "Wald", joint=F)
    }, silent = T)
    iter <- iter+1
  }
  if (!is.null(obj)) {
    ...
    df_qr_test[df_qr_test$Country==kraj,12] <- NA
    obj <- NULL
  }
  iter=0
  i<- i+1
...
for (x in kraje) {
  kraj <- kraje1[i]
  for (q in seq(0.1, 0.8, 0.1)) {
    q_reg <- rq(ree_r~oilprice_r_p+oilprice_r_n+D08+D20, tau = c(q,q+0.1), data=data[[x]][-1,] %>% mutate(ree_r=ree_r*100))
    try({
      obj <-anova(q_reg, test = "Wald", joint=F)
    }, silent = T)
    iter=0
    while(is.null(obj) & iter<200) {
      try({
        q_reg <- rq(ree_r~oilprice_r_p+oilprice_r_n+D08+D20, tau = c(q+runif(1, -0.02, 0.02),q+0.1+runif(1, -0.04, 0.04)), data=data[[x]][-1,] %>% mutate(ree_r=ree_r*100))
        obj <-anova(q_reg, test = "Wald", joint=F)
      }, silent = T)
      iter <- iter+1
    }
    if (!is.null(obj)) {
      ...
      df_qr_test_asm[df_qr_test_asm$Country==kraj,10*q+2] <- NA
      obj <- NULL
    }
    iter=0
    cat('q=',q)
...
for (x in kraje) {
  ...
  while(is.null(obj) & iter<200) {
    try({
      q_reg <- rq(ree_r~oilprice_r_p+oilprice_r_n+D08+D20, tau = c(0.1+runif(1, -0.02, 0.02),0.5+runif(1, -0.03, 0.03)), data=data[[x]][-1,] %>% mutate(ree_r=ree_r*100))
      obj <-anova(q_reg, test = "Wald", joint=F)
    }, silent = T)
    iter <- iter+1
  }
  if (!is.null(obj)) {
    ...
    df_qr_test_asm[df_qr_test_asm$Country==kraj,11] <- NA
    obj <- NULL
  }
  iter=0
  i<- i+1
...
for (x in kraje) {
  ...
  while(is.null(obj) & iter<200) {
    try({
      q_reg <- rq(ree_r~oilprice_r_p+oilprice_r_n+D08+D20, tau = c(0.5+runif(1, -0.02, 0.02),0.9+runif(1, -0.04, 0.04)), data=data[[x]][-1,] %>% mutate(ree_r=ree_r*100))
      obj <-anova(q_reg, test = "Wald", joint=F)
    }, silent = T)
    iter <- iter+1
  }
  if (!is.null(obj)) {
    df_qr_test_asm[df_qr_test_asm$Country==kraj,12] <- round(obj$table$pvalue,4)
    obj <- NULL  
  } else {
    df_qr_test_asm[df_qr_test_asm$Country==kraj,12] <- NA
    obj <- NULL
  }
  iter=0
  i<- i+1
